...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_POST['HoTen']) && !empty($_POST['ChucVu']) && 
        !empty($_POST['SoDienThoai']) && !empty($_POST['Email']) &&
        !empty($_POST['Address']) && !empty($_POST['TenDangNhap']) && 
        !empty($_POST['MatKhau'])) {
        ...
        $sqlUser = "INSERT INTO nguoidung (SDT, TenDangNhap, MatKhau, QuyenHan) VALUES ( ?, ?, ?, 'NhanVien')";
        $stmtUser = $conn->prepare($sqlUser);
        $stmtUser->bind_param("sss", $SoDienThoai, $TenDangNhap, $MatKhau);
        
        if ($stmtUser->execute()) {
            $MaND = $stmtUser->insert_id; 

            $sqlEmployee = "INSERT INTO nhanvien (HoTen, ChucVu, SDT, Email, DiaChi, MaND) VALUES (?, ?, ?,?,?,?)";
            $stmtEmployee = $conn->prepare($sqlEmployee);
            $stmtEmployee->bind_param("sssssi", $HoTen, $ChucVu, $SoDienThoai, $Email, $Address, $MaND);
                ...
                <div class="box">
                    ...
                    <div class="dauvao">
                        ...
                        <div class="info">
                            ...
                            <input type="email" id="Email" name="Email" value="" placeholder="Nhập email" style="opacity: 0.6;">
                            <span style=" font-size: 18px; color: red; padding-left: 20px; display: none;" class="error" id="EmailError">Email không được để trống</span>
                        </div>

                        <div class="info">
                            <label for="">Địa chỉ: </label>
                            <input type="text" id="Address" name="Address" value="" placeholder="Nhập địa chỉ" style="opacity: 0.6;">
                            <span style=" font-size: 18px; color: red; padding-left: 20px; display: none;" class="error" id="AddressError">Địa chỉ không được để trống</span>
                        </div>
    ...
    <script>
        ...
        formThemNV.addEventListener("submit", function (e) { 
            ...
            if (TenDangNhap.value.trim() === "") {
                check = false;
                document.getElementById('TenDangNhapError').style.display = "block";
            }

            if (MatKhau.value.trim() === "") {
                check = false;
                document.getElementById('MatKhauError').style.display = "block";
            }
